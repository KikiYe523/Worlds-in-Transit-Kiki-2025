...
export default class Model2 {
  ...
  init() {
    ...
    this.loader.load(this.file, (gltf) => {
      ...
      gltf.scene.traverse((child) => {
        if (child.isMesh && child.name === "Lights_lights_0") {
          child.material.emissive = new THREE.Color(0xffffff);
      ...
      //center the model's pivot point to its bounding box center
      ...
      // const box = new THREE.Box3().setFromObject(this.meshes[`${this.name}`]);
      // const center = new THREE.Vector3();
      // box.getCenter(center);
      // this.meshes[`${this.name}`].position.sub(center); // 模型的本地原点移动到包围盒中心
      // const boxHelper = new THREE.BoxHelper(
      //   this.meshes[`${this.name}`],
      //   0xffff00
      // );
