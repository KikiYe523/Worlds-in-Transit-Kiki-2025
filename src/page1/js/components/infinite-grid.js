...
// import bg from "../../static/img/page1/bg1.mp4";
export default class InfiniteGrid {
  ...
  createVideoBackground() {
    ...
    this.videoBg.src = `${import.meta.env.VITE_MODEL_PATH_PREFIX}/bg/bg1.mp4`;
      ...
      repsX.forEach((offsetX) => {
        repsY.forEach((offsetY) => {
          ...
          if (base.type === "model") {
            ...
            console.log("[InfiniteGrid] Creating video element with src:", base.src);

            itemImage.appendChild(videoEl);

            // 加载完数据后，静默播放并立即暂停，以强制渲染首帧为封面
            const showFirstFrame = () => {
              console.log("[InfiniteGrid] showFirstFrame called, readyState:", videoEl.readyState);

              // 确保视频有数据可以渲染
              if (videoEl.readyState >= 2) {
                // HAVE_CURRENT_DATA or higher
                console.log("[InfiniteGrid] Video ready, attempting to show first frame");
                ...
                if (playPromise && typeof playPromise.then === "function") {
                  playPromise
                    .then(() => {
                      console.log("[InfiniteGrid] Video played successfully, pausing now");
                      // 播放成功后立即暂停在第一帧
                      setTimeout(() => {
                        videoEl.pause();
                        try {
                  ...
                  console.log("[InfiniteGrid] No promise returned, pausing directly");
                  videoEl.pause();
                }
              } else {
                console.log("[InfiniteGrid] Video not ready yet, readyState:", videoEl.readyState);
              }
            };
            ...
            videoEl.addEventListener("loadedmetadata", () => {
              console.log("[InfiniteGrid] loadedmetadata event fired for:", base.src);
            }, { once: true });

            videoEl.addEventListener("loadeddata", () => {
              console.log("[InfiniteGrid] loadeddata event fired for:", base.src);
              showFirstFrame();
            }, { once: true });

            videoEl.addEventListener("canplay", () => {
              console.log("[InfiniteGrid] canplay event fired for:", base.src);
            }, { once: true });

            // 如果视频已经加载完成，立即执行
            setTimeout(() => {
              if (videoEl.readyState >= 2) {
                console.log("[InfiniteGrid] Video already loaded, showing first frame immediately");
                showFirstFrame();
              }
            }, 100);
          ...
          const itemData = {
            ...
            detail: this.sources[this.data.findIndex(d => d.x === base.x && d.y === base.y)].detail,
  ...
  onItemClick(itemData) {
    ...
    const targetX = containerRect.left + containerRect.width / 2 - elementRect.width / 2;
    const targetY = containerRect.top + containerRect.height / 2 - elementRect.height / 2;
    ...
    tl.to(animatedElement, {
      ...
      onComplete: () => {
        ...
        this.textAnimations.fromTo(this.detailTextSplit.chars,
          {
            opacity: 0,
            y: 20
          },
        ...
        this.textAnimations.fromTo(this.captionTextSplit.lines,
          {
            opacity: 0,
            y: 30
          },
        ...
        const checkAndShowCloseButton = (mouseX, mouseY) => {
          ...
          const isOutsidePanel = mouseX < panelRect.left ||
  ...
  onCloseButtonClick() {
          ...
          onComplete: () => {
            ...
            if (this.activeItemIsCloned) {
              ...
              if (this.originalCanvasStyles && this.activeItemClone) {
                this.activeItemClone.style.position = this.originalCanvasStyles.position;
                this.activeItemClone.style.left = this.originalCanvasStyles.left;
                this.activeItemClone.style.top = this.originalCanvasStyles.top;
                this.activeItemClone.style.width = this.originalCanvasStyles.width;
                this.activeItemClone.style.height = this.originalCanvasStyles.height;
                this.activeItemClone.style.zIndex = this.originalCanvasStyles.zIndex;

                // Restore to original parent
                if (this.originalCanvasStyles.parent) {
                  this.originalCanvasStyles.parent.appendChild(this.activeItemClone);
                }
    ...
    if (this.detailTextSplit && this.detailTextSplit.chars) {
      textExitTl.to(this.detailTextSplit.chars, {
        opacity: 0,
        y: -20,
        duration: 0.03,
        stagger: 0.01,
    ...
    if (this.captionTextSplit && this.captionTextSplit.lines) {
      textExitTl.to(this.captionTextSplit.lines, {
        opacity: 0,
        y: -30,
        duration: 0.4,
        stagger: 0.08,
